    ...
    export class NinaController extends net.Controller {
        ...
        */
        ...
        private _sendCommand(cmd: number, params?: Buffer[], param_len_16?: boolean) {
            ...
            // padding
            while (n % 4) n++;
            OS.Debug.crashLikeDebug("Making buffer");
            const packet = control.createBuffer(n);
            OS.Debug.crashLikeDebug("Made buffer");
            let k = 0;
            packet[k++] = _START_CMD;
            ...
            params.forEach(param => {
                OS.Debug.crashLikeDebug("Writing to packet");
                if (param_len_16)
        ...
        private spiTransfer(tx: Buffer, rx: Buffer) {
            ...
            OS.Debug.crashLikeDebug("Transfering cmd");
            if (!this._spi) {
                OS.Debug.panic("SPI not configured");
            };

            this._spi.transfer(tx, rx);
            OS.Debug.crashLikeDebug("Done!");
        };

        private _waitResponseCmd(cmd: number, num_responses?: number, param_len_16?: boolean) {
            net.debug(`wait response cmd`);
            OS.Debug.crashLikeDebug("Wait response cmd");
            //if (!this.waitForReady())
            //    return null

            this._cs.digitalWrite(false)
            OS.Debug.crashLikeDebug("_CS digital write off");
            ...
            OS.Debug.crashLikeDebug("Checking data");
            this.checkData(cmd | _REPLY_FLAG)
            OS.Debug.crashLikeDebug("Checked data");
            ...
            for (let num = 0; num < num_responses; ++num) {
                ...
                OS.Debug.crashLikeDebug(`Parameter #${num} length is ${param_len}`);
        ...
        private sendCommandGetResponse(cmd: number, params?: Buffer[],
            ...
            OS.Debug.crashLikeDebug("Locking");
            this.lock()
            OS.Debug.crashLikeDebug("Sending cmd");
            this._sendCommand(cmd, params, sent_param_len_16)
            OS.Debug.crashLikeDebug("Getting response");
...
*/
    ...
    */
        public scanNetworks(): net.AccessPoint[] {
            OS.Debug.crashLikeDebug("Scan networks");
            // this.startScanNetworks();
            OS.Debug.crashLikeDebug("Started Scan");
            // attempts
            for (let _ = 0; _ < 10; ++_) {
                pause(2000)
                // pylint: disable=invalid-name
         ...
         */
        public connect(timeout?: number): boolean {
            ...
            for(const network of networks) {
                if(this.connectAPd(network.ssid, wifis[network.ssid]) == WL_CONNECTED)
                    return true;
            }
        ...
        */
        private connectAPd(ssid: string, password: string): number {
...
namespace newsp32 {
    ...
    export function defaultController(): net.Controller {
        ...
        if (!!cs) {
            ...
            if (!pins.pinByCfg(DAL.CFG_PIN_MISO)) {
                OS.Debug.success("Falling back to MISO for CFG_PIN_MISO");
            }
            if (!pins.pinByCfg(DAL.CFG_PIN_SCK)) {
                OS.Debug.success("Falling back to SCK for CFG_PIN_SCK");
            }
            if (!mosi || !miso || !sck) {
                if (!mosi) {
                    OS.Debug.fail("WIFI IO mosi");
                };
                if (!miso) {
                    OS.Debug.fail("WIFI IO miso");
                };
                if (!sck) {
                    OS.Debug.fail("WIFI IO sck");
                };
                OS.Debug.panic("WIFI IO");
            };
            ...
            for (let i = 82; i < 93; i++) {
                try {
                    let pin = pins.pinByCfg(i);
                    if (pin) {
                        OS.Debug.success(`Pin ${i}`);
                    } else {
                        OS.Debug.warn(`Pin ${i}`);
                    };
                } catch (e) {
        ...
        // no option
        OS.Debug.panic("newsp32 configuration");
        return undefined;
    };
};
...
namespace net {
    export class Configurator {
        ...
        main() {
            this.wifi = new Net(newsp32.defaultController).controller;
            if (!this.wifi) {
